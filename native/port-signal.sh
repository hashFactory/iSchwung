#!/bin/bash
# port-signal.sh - Builds the signal module (synth) for Apple targets and stages it
# into build/external for sync-runtime.sh.
set -e

NATIVE_DIR="$(cd "$(dirname "$0")" && pwd)"
SRC="$NATIVE_DIR/build/ports/signal-src"
DEST="$NATIVE_DIR/build/external/sound_generators/signal"
...
case "$TARGET" in
    ...
    iossim) SDK="$(xcrun --sdk iphonesimulator --show-sdk-path)"
            ...
            DSP_OUT="$NATIVE_DIR/build/iossim/external/sound_generators/signal/dsp.so" ;;
    ios)    SDK="$(xcrun --sdk iphoneos --show-sdk-path)"
            TARGET_FLAGS="-target arm64-apple-ios16.0 -isysroot $SDK"
            DSP_OUT="$NATIVE_DIR/build/ios/external/sound_generators/signal/dsp.so" ;;
    *) echo "unknown TARGET=$TARGET"; exit 1 ;;
esac

if [ ! -d "$SRC" ]; then
    git clone --depth 1 https://github.com/filliformes/signal-move "$SRC"
fi

mkdir -p "$DEST" "$(dirname "$DSP_OUT")"

echo "DSP signal ($TARGET)"
