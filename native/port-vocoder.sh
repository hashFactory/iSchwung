#!/bin/bash
# port-vocoder.sh - Builds the schwung-vocoder module (vocoder FX) for Apple
# targets and stages it into build/external for sync-runtime.sh.
set -e

NATIVE_DIR="$(cd "$(dirname "$0")" && pwd)"
SRC="$NATIVE_DIR/build/ports/vocoder-src"
DEST="$NATIVE_DIR/build/external/audio_fx/vocoder"
...
case "$TARGET" in
    ...
    iossim) SDK="$(xcrun --sdk iphonesimulator --show-sdk-path)"
            ...
            OUT_DIR="$NATIVE_DIR/build/iossim/external/audio_fx/vocoder" ;;
    ios)    SDK="$(xcrun --sdk iphoneos --show-sdk-path)"
            TARGET_FLAGS="-target arm64-apple-ios16.0 -isysroot $SDK"
            OUT_DIR="$NATIVE_DIR/build/ios/external/audio_fx/vocoder" ;;
    *) echo "unknown TARGET=$TARGET"; exit 1 ;;
esac

if [ ! -d "$SRC" ]; then
    git clone --depth 1 https://github.com/charlesvestal/schwung-vocoder "$SRC"
fi

mkdir -p "$DEST" "$OUT_DIR"

echo "DSP vocoder ($TARGET)"
